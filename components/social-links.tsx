...
const socialLinks = [
  ...
  {
    name: "Bluesky",
    href: "https://bsky.app/profile/fullstackdrip.bsky.social",
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor">
...
function SocialLink({ link }: { link: (typeof socialLinks)[0] }) {
  ...
  return (
    <motion.a
      ...
      whileHover="hover"
    >
      <div className="flex h-14 w-14 items-center justify-center rounded-full bg-foreground/10 text-foreground transition-colors hover:bg-primary hover:text-primary-foreground">
        <div className="h-7 w-7">{link.icon}</div>
      </div>
      ...
      <motion.div
        ...
        className="pointer-events-none absolute left-1/2 top-full mt-2 -translate-x-1/2 whitespace-nowrap rounded bg-brand-blue-light px-2 py-1 text-xs font-medium text-white"
      >
        {link.name}
        {/* Arrow */}
        <div className="absolute -top-1 left-1/2 h-2 w-2 -translate-x-1/2 rotate-45 bg-brand-blue-light" />
      </motion.div>
    </motion.a>
  )
}
